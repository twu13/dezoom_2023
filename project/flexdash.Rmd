...
DT::renderDataTable({
  ...
  temp_phys <- data_phys %>%
    ...
    dplyr::filter(recipient_state %in% input$select_state)
  final <- temp_state %>%
    dplyr::summarise(val = scales::comma(sum(total_payments))) %>%
    dplyr::mutate(met = "Total Payments Made: ") %>%
    dplyr::bind_rows(temp_state %>%
      dplyr::summarise(val = scales::dollar(sum(total_pay_usd))) %>%
      dplyr::mutate(met = "Total Payments in USD: ")) %>%
    dplyr::bind_rows(temp_mfc %>%
      dplyr::summarise(val = scales::comma(length(unique(temp_mfc$mfc_gpo)))) %>%
      dplyr::mutate(met = "Total Manufacturers Making Payments: ")) %>%
    dplyr::bind_rows(temp_phys %>% dplyr::group_by(recipient_state) %>% dplyr::summarise(total = sum(total_pay_usd)) %>% dplyr::arrange(desc(total)) %>% dplyr::slice(1) %>% dplyr::select(val = recipient_state) %>%
      dplyr::mutate(met = "State Receiving the Most Money in Physician Payments: ")) %>%
    dplyr::bind_rows(temp_phys %>% dplyr::group_by(recipient_state) %>% dplyr::summarise(total = sum(total_payments)) %>% dplyr::arrange(desc(total)) %>% dplyr::slice(1) %>% dplyr::select(val = recipient_state) %>%
      dplyr::mutate(met = "State Receiving the Highest Number of Physician Payments: ")) %>%
    dplyr::select(met, val)
  return(DT::datatable(final,
    options = list(
      headerCallback = JS(
